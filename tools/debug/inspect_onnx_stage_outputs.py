...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Inspect intermediate ONNX stage outputs against a baseline.")
...
def run_onnx(weights: Path, image_tensor: np.ndarray) -> np.ndarray:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "If it contains Horizon custom ops, inspect it inside the Horizon/OpenExplorer environment."
        ) from exc
...
def summarize(name: str, canonical: np.ndarray, topk: int) -> list[int]:
    ...
    print(f"[{name}] shape={tuple(canonical.shape)} score min/max/mean={score_channel.min():.6f}/{score_channel.max():.6f}/{score_channel.mean():.6f}")
